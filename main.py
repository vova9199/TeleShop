...
def message_send(message):
    if '/start' == message.text:
        if message.chat.username:
            ...
            if message.chat.id == config.admin_id and dop.it_first(message.chat.id) is True:
                ...
                key.add(telebot.types.InlineKeyboardButton(text='Перейти к каталогу товаров',
        ...
        if sost_num == 22:
            ...
            try:
                ...
                key.add(
                    telebot.types.InlineKeyboardButton(text='Вернуться в начало', callback_data='Вернуться в начало'))
...
def inline(callback):
    ...
    if callback.message.chat.id in in_admin:
        ...
        if dop.get_productcatalog() == None:
            ...
            try:
                bot.edit_message_text(chat_id=callback.message.chat.id, message_id=callback.message.message_id,
                                      text=dop.get_productcatalog(), reply_markup=key, parse_mode='Markdown')
            except:
                pass
        ...
        if dop.amount_of_goods(name_good) == 0:
            ...
            try:
                bot.edit_message_text(chat_id=callback.message.chat.id, message_id=callback.message.message_id,
                                      text='Введите *количество* товаров к покупке\n*Минимальное* количество к покупке: ' + str(
                                          dop.get_minimum(name_good)) + '\n*Максимальное* доступное: ' + str(
                                          dop.amount_of_goods(name_good)), reply_markup=key, parse_mode='Markdown')
            except:
                pass
            with shelve.open(files.sost_bd) as bd:
        ...
        payments.check_oplata_qiwi(callback.message.chat.id, callback.from_user.username, callback.id,
                                   callback.message.from_user.first_name)

    elif callback.data == 'Проверить оплату btc':
        payments.check_oplata_btc(callback.message.chat.id, callback.from_user.username, callback.id,
                                  callback.message.from_user.first_name)
