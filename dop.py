...
*Полный гайд по настройке бота*(рекомендую ознакомиться) - https://vk.cc/avGFHJ
...
def get_productcatalog():
	...
	for name, description, price, stored in cursor.fetchall():
		...
		product_list += '*' + name +  '*' + ' `-`' + '  ' + lastprice + '  ' + ' *' + str(price) + '*' + ' ₽ ' + '(~' + str(round(float(price)/2.65, 2)) + ' ₴) ' + '(Осталось ' + str(good_amount) +'шт.)\n'
	con.close()
	if a == 0: return None
